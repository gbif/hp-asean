...
# SQL query for GBIF download - using publishingCountry instead of countryCode
sql <- "
SELECT 
  publishingCountry,
  \"year\",
...
existing_downloads <- list.files(pattern = "^publisher-[0-9]+-[0-9]+\\.zip$", full.names = TRUE)
...
if (length(existing_downloads) > 0) {
  ...
  download_key <- sub("^publisher-", "", sub("\\.zip$", "", basename(download_file)))
  ...
  # Get the download and rename it
  cat("Downloading results...\n")
  data <- rgbif::occ_download_get(download_key) |>
    rgbif::occ_download_import()
  
  # Rename the downloaded file to include "publisher-" prefix
  if (file.exists(paste0(download_key, ".zip"))) {
    file.rename(paste0(download_key, ".zip"), paste0("publisher-", download_key, ".zip"))
  }
}
...
data <- data %>% filter(publishingcountry %in% TARGET_COUNTRIES)

# Get unique countries in the dataset
countries_with_data <- unique(data$publishingcountry)
...
process_country_data <- function(country_code, country_data) {
  ...
  # Build the full country payload with PUBLISHER dataType
  payload <- list(
  ...
  # Save to JSON file
  json_file <- file.path(OUTPUT_DIR, paste0(country_code, "-publisher.json"))
...
for (cc in countries_with_data) {
  country_data <- data %>% filter(publishingcountry == cc)
